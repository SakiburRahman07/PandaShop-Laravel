...
class CompareController extends Controller
{
    public function AddToCompare(Request $request, $product_id){
      ...
      $exists = Compare::where('user_id',session('user_id'))->where('product_id',$product_id)->first();
    ...
    public function GetCompareProduct(){
        ...
        $compare = Compare::with('product')->where('user_id',session('user_id'))->latest()->get(); 
    ...
    public function CompareRemove($id){
        ...
        Compare::where('user_id',session('user_id'))->where('id',$id)->delete();
