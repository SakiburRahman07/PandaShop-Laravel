            ...
            <tbody>
               @foreach($carts as $item) 
                <tr>
                    ...
                    <td>
                        <h6 class="w-160 mb-5"><a href="shop-product-full.html" class="text-heading">{{ $item->product_name }}</a></h6></span>
...
<script type="text/javascript">
    ...
    $('select[name="district_id"]').on('change', function(){
        ...
        if (district_id) {
            $.ajax({
                url: "/state-get/ajax/" + district_id, // Use correct URL without Blade templating
                type: "GET",
                dataType: "json",
                success: function(data){
                    $('select[name="state_id"]').html('');
